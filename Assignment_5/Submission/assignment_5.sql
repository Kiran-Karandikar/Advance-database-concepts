...
-- skills.
with rp as (select array_agg(ps.pid) as pids from personHasSkills ps  where is_in('AI', ps.skills) and is_in('Programming', ps.skills)) select c.* from company c where c.cname in (select ce.cname from companyHasEmployees ce, rp  where  cardinality (setintersection(ce.employees, rp.pids))>=2) order by c.cname;

\qecho 'Problem 1.b'  
-- Find each skill that is not a job skill of any person who works for
-- Yahoo or for Netflix.
with rc as (select ce.employees as pids from companyHasEmployees ce where ce.cname in ('Yahoo', 'Netflix')),
all_employee as ( select array_agg(employees) as pids from rc, unnest(rc.pids) employees), skill_records as (
...
with all_people as (select array_agg(kp.pid) as pids from knowsPersons kp where cardinality(kp.persons)>=5)
...
with known_by as (select p.pid, p.pname, ikbp.persons as persons from person p, isKnownByPersons ikbp where  ikbp.pid=p.pid), rp as (select array_agg(ps.pid) as pids from personHasSkills ps where is_in('AI', ps.skills) and  not is_in('Programming', ps.skills)) select kb.pid, kb.pname, setintersection(kb.persons, rp.pids) from known_by kb, rp;
...
with mores as (select array_agg(w.pid) as pids from worksfor w where w.salary> 55000),
pair_1 as (select sp.skill, cardinality(set_intersection(sp.persons, mp.pids)) as count from skillOfPersons sp, mores mp), less as (select array_agg(w.pid) as pids from worksfor w where w.salary<= 55000),
pair_2 as (select sp.skill, cardinality(set_intersection(sp.persons, lp.pids)) as count from skillOfPersons sp, less lp) select p1.skill, p2.skill from pair_1 p1, pair_2 p2 where p1.count < p2.count and p1.skill <> p2.skill;
...
with base as ( select make_singleton(s1.skill) as sskill, remove_element(s1.skill, make_singleton(s1.skill)) as eskill, (select distinct set_union(make_singleton(s1.skill), make_singleton(s2.skill))) as twoskill
...
create temp view jarrod_values as ( select s1.skill as skill1, s2.skill as skill2,
(cast(cardinality(setintersection(s1.persons, s2.persons)) as float)/cast (cardinality(setunion(s1.persons, s2.persons)) as float)) as j_index from skillOfPersons s1, skillOfPersons s2 where s1.skill <> s2.skill);
...
with all_sids as (select cno, grade, array_agg(row(sid)::stduentsType) as sids from enroll group by cno, grade
...
-- students:(sid:text)[]).
with base_q as (select cg.cno, unnest(gi.sid) as sids from courseGrades cg, unnest(gradeinfo) gi where gi.grade in ('A', 'B'))
...
with base_q as (select cg.cno, sids.sid from courseGrades cg, unnest(gradeinfo) gi, unnest(gi.sid) sids where gi.grade='A') select bq.sid as sid, array_agg(row(bq.cno)::courseInfoType order by bq.cno) as courses from base_q as bq group by sid order by 1;
...
with base_q as (select cno, grade, jsonb_agg(jsonb_build_object('sid', sid) order by sid) as js from enroll group by cno, grade) select jsonb_build_object('cno', cno, 'gradeInfo', jsonb_agg(jsonb_build_object('grade', grade, 'students', bq.js)))   as courseinfo from base_q bq group by cno order by cno);
...
select * from studnetgradedetails('B');	
\qecho 'Grade = A'

select * from studnetgradedetails('A');
\c postgres
