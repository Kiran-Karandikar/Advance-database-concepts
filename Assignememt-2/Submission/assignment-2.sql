...
select distinct p.pid , p.pname from person p, worksfor w where exists (select 1 from worksfor wf1 where wf1.cname='Google' and wf1.pid=p.pid) and exists (SELECT 1
	FROM knows kn where kn.pid1=p.pid and exists(select 1 from worksfor wf where wf.cname='Google' and wf.pid=kn.pid2 and wf.salary < w.salary));
